        ...
        : MPDTransportController(ctlConfig)
{
    ...
    // vegas
    vegasccConfig.minCwnd = m_transCtlConfig->minWnd;
    vegasccConfig.maxCwnd = m_transCtlConfig->maxWnd;
    // [kyl] end
...
void DemoTransportCtl::OnSessionCreate(const fw::ID& sessionid)
{
    ...
    if (sessionItor == m_sessStreamCtlMap.end())
    {
        ...
        m_sessStreamCtlMap[sessionid]->StartSessionStreamCtl(sessionid, vegasccConfig,shared_from_this());
        // [kyl] end
    }
    else
    {
