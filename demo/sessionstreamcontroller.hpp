...
public:
    ...
    // [kyl] begin
    // void StartSessionStreamCtl(const basefw::ID& sessionId,RenoCongestionCtlConfig& ccConfig,std::weak_ptr<SessionStreamCtlHandler> ssStreamHandler)
    void StartSessionStreamCtl(const basefw::ID& sessionId,VegasCongestionCtlConfig& ccConfig,std::weak_ptr<SessionStreamCtlHandler> ssStreamHandler)
    {
        if (isRunning)
        {
        ...
        m_congestionCtl.reset(new VegasCongestionContrl(m_ccConfig));
        // [kyl] end
...
private:
    ...
    VegasCongestionCtlConfig m_ccConfig;
    // [kyl] end
